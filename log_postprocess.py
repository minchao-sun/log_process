...
class ConfigParser(object):
    ...
    def getProblemFiles(self, problemtype):
        files = []
        ffs = None
        for p in self.root.find("problemneededfile").findall("problem"):
...
class Main:
    def __init__(self, vmlinuxPath, sysdumpPath=None, slogPath=None, devbitInfo=None, devNum=None):
        ...
        self.filesmap = {}
        self.fzs = []
        if self.sysdumpPath is not None:
            self.initData()
        if platform.system() == "Linux":
    ...
    def genReport(self):
        ...
        print "detail pls refer to the report"
    ...
    if os.path.exists(sd_path + os.path.sep + 'MSR.log'):  # reboot_test:
        ...
        for keyword in keyword_list:
            ...
            if et:
                ...
                result_flag = True
                break
        if result_flag is False and os.path.exists(lowpower_path):
            if sd_kernel_file_exist_flag is True:
...
def get_nativecrash_info(tombsf, pl, pr):
    ...
    if os.path.exists(tombsf):
        ...
        return 0
    line = fd.readline()
    while line.find("ylog.tombstone 000") == -1:
        line = fd.readline()
...
def parse_native_crash(logdir, devnum):
    ...
    if logdir is None:
        ...
        if reportpath == '' or reportpath is None:
...
def parse_lowpower(input_dir, devnum):
    ...
    if input_dir is None:
        ...
        if sd_kernel_file_exist_flag:
            ...
            if android_main_file_exist_flag is True:
                for sd_last_android_main_file in last_android_main_file:
